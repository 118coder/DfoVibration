...
pub fn load_fonts(ctx: &egui::Context, language: Language) {
    ...
    // Phosphor 图标字体 (统一图标语言, 替代 emoji/手绘)
    egui_phosphor::add_to_fonts(&mut fonts, egui_phosphor::Variant::Regular);

    // 真粗体字体族 "Bold": egui 的 RichText::strong 只是变色不是加粗,
    // 标题/重点标签需要真实字重 → 独立族 (Segoe UI Bold 拉丁 + 雅黑 Bold 中文)
    let bold_fonts = [
        ("SegoeUI-Bold", "C:/Windows/Fonts/segoeuib.ttf"),
        ("YaHei-Bold", "C:/Windows/Fonts/msyhbd.ttc"),
    ];
    for (name, path) in bold_fonts {
    ...
    // 粗体族补齐与默认族相同的回退链 (Phosphor 图标 + CJK + emoji/符号),
    // 否则加粗文本里的图标/中文/符号会渲染成 "?" — 直接复制 Proportional 的回退
    let prop_fallbacks: Vec<String> = fonts
        .families
        .get(&egui::FontFamily::Proportional)
        .map(|prop| prop.iter().skip(1).cloned().collect())
        .unwrap_or_default();
    if let Some(keys) = fonts.families.get_mut(&egui::FontFamily::Name("Bold".into())) {
        for f in prop_fallbacks {
            // 跳过默认族主字体 (Ubuntu-Light, 与粗体风格不符) 已由 skip(1) 处理
            if !keys.contains(&f) {
                keys.push(f);
            }
        }
