...
### 2.1 深色主题 (Graphite Indigo · v5)

| Token | 值 | 用途 |
|---|---|---|
| `bg` | `#0C0F15` | 应用底色 (内容区最深层) |
| `surface` | `#131720` | 侧边栏 / 顶栏 / 弹窗底 |
| `card` | `#1B202B` | 卡片面 |
| `card_alt` | `#222835` | 卡片内嵌套面板 |
| `faint` | `#2D3445` | 悬停底 / 次级按钮底 |
| `extreme` | `#07090D` | 输入框 / 键帽底 |
| `stroke` | `#FFFFFF1A` (白 10%) | hairline 分隔线 / 卡片描边 |
| `stroke_strong` | `#FFFFFF38` (白 22%) | 悬停描边 / 浮层描边 |
| `accent` | `#818CF8` | 强调填充 (滑块/指示条/图标/选中文字) |
| `accent_hover` | `#A5B4FC` | 强调悬停 |
| `accent_soft` | `#818CF8` @18% (alpha 46) | 选中软底 / 柔底徽章 |
| `accent_text` | `#A5B4FC` | 深底上的强调文字/链接 (card 上 9.0:1) |
| `title` | `#F5F7FA` | 一级标题 |
| `heading` | `#E4E8EF` | 二级标题 |
| `text` | `#C6CCD8` | 正文 (card 上 10.1:1) |
| `text_weak` | `#9BA3B2` | 次要文字 |
| `hint` | `#8C94A4` | 提示文字 (card 上 5.4:1 / 嵌套面板 4.8:1) |
| `good` | `#34D399` | 成功/运行中 |
| `bad` | `#FB7185` | 危险/错误 (rose-400, 中性底上更柔) |
| `warn` | `#FBBF24` | 警告/暂停 |
| `info` | `#38BDF8` | 信息 |
| `btn_primary` | `#4F55E8` | 主按钮深档 (白字 5.5:1, 与亮色主题同色) |
| `btn_danger` | `#E11D48` | 危险按钮 |
| `trigger_fg/bg` | `#FCD34D` @10% | 触发键徽章 (琥珀) |
| `target_fg/bg` | `#7DD3FC` @9% | 目标键徽章 (天蓝) |
| `motor_l` | `#FB923C` | 左马达 |
| `motor_r` | `#F87171` | 右马达 |

> ⚠ 改深色色板后必跑 `work/_palette_check.py` 复核 (文字三层对比度 + 层间步进)。
> 用色纪律: 大面积行动区用 `btn_primary` (深档); 徽章/选中态用 `accent_text + accent_soft`;
> `accent` (亮档) 只用于小元素 (图标/指示条/滑块/描边) —— 亮档大面积饱和填充会发飘。
...
## v3.2 增补 (2026-09-07) — 按钮语义层级 (强制)

| 档位 | 令牌 | 用途 | 例 |
|---|---|---|---|
| Primary | `btn_primary` (accent 填充 + 白字) | 每屏 1-2 个的最常用动作 | 保存 / 新增映射 / 确认 |
| Secondary | `btn_secondary` + `btn_secondary_text` | 一般操作 / 取消 | 编辑 / 取消 / 导入导出 |
| Danger | `btn_danger` | 破坏性动作 (常配二次确认, 未激活时可降级为 faint) | 删除预设 / 删除映射 |
| Ghost | `ghost_button` (accent_soft) | 次要强调入口 | 徽章跳转类 |
| 激活态 | `accent` | 瞬时激活指示 | 按键捕获中 |

规则 (v3.2 起强制):
1. 语义色 good/bad/warn 只表示"状态", 永远不用作确认/取消按钮的填充
...
## v5 深色重制 (2026-09-21) — "Graphite Indigo 石墨靛蓝"

**触发**: 用户反馈"深色页面不好看" (附截图)。诊断三条:
① v4 三阶灰**全部带紫调** (色相 ~250°, 饱和 ~25%) —— bg/card/文字彼此只差明度,
色相相同 → 整屏糊成一片紫灰, 层级感全靠明度硬拉;
② 强调紫以**大面积饱和块**出现 (大按钮/选中页签/白字压紫底徽章) → 视觉噪音大, 泛"廉价霓虹"感;
③ 语义色同时亮着 4 个色相 (绿/琥珀/天蓝/粉) + 紫底, 颜色竞争。

**方案** (v5, 仅动深色; 亮色保持蓝白基调不变):
- 骨架改**中性冷灰** (色相 ~220°, 饱和 <15%): bg #0C0F15 → card #1B202B → card_alt #222835
  → faint #2D3445; 层间步进 1.07~1.19 (比 v4 的 1.04~1.12 更清晰), 描边升到白 10%/22%;
- 强调改**靛蓝族**, 与亮色主题同色系: accent #818CF8 (只作小元素) / accent_text #A5B4FC
  (徽章文字) / accent_soft (柔底) / **btn_primary #4F55E8 深档** (大面积行动区, 白字 5.5:1);
- 语义色收敛: bad 由 rose-500 #F43F5E → rose-400 #FB7185 (中性底上更柔);
- 用色纪律 (新增, 已写入 §2.1 与 theme.rs 头注释): 亮档 accent 禁止大面积饱和填充;
- 复核工具: `work/_palette_check.py` —— 文字三层对比度 (正文≥7 / 小字≥4.5) + 层间步进 ≥1.07,
  改色板后必跑。
