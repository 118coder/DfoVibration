...
fn get_raw_translation(lang: Language, key: RawKey) -> &'static str {
    match (lang, key) {
        ...
        (Language::English, RawKey::DoubleTapOnHover) => "Double-tap ON - one press sends two taps",
        (Language::English, RawKey::DoubleTapOffHover) => "Double-tap OFF - one press sends one tap",
        (Language::SimplifiedChinese, RawKey::TurboOffHover) => "连发关闭 - 仅单次输入",
        (Language::SimplifiedChinese, RawKey::DoubleTapOnHover) => "双击开启 - 按一次触发两次按键(DFO跑动)",
        (Language::SimplifiedChinese, RawKey::DoubleTapOffHover) => "双击关闭 - 按一次触发一次按键",
        (Language::TraditionalChinese, RawKey::TurboOffHover) => "連發關閉 - 僅單次輸入",
        (Language::TraditionalChinese, RawKey::DoubleTapOnHover) => "雙擊開啟 - 按一次觸發兩次按鍵(DFO跑動)",
        (Language::TraditionalChinese, RawKey::DoubleTapOffHover) => "雙擊關閉 - 按一次觸發一次按鍵",
        (Language::Japanese, RawKey::TurboOffHover) => "連打オフ - 単発入力",
        (Language::Japanese, RawKey::DoubleTapOnHover) => "ダブルタップON - 1回で2回入力",
        (Language::Japanese, RawKey::DoubleTapOffHover) => "ダブルタップOFF - 1回で1回入力",
