...
struct TranslationCache {
    ...
    double_tap_on_hover: String,
    double_tap_off_hover: String,
...
impl CachedTranslations {
    ...
    pub fn double_tap_on_hover(&self) -> &str {
        &self.inner.double_tap_on_hover
    }
    pub fn double_tap_off_hover(&self) -> &str {
        &self.inner.double_tap_off_hover
    }
    ...
    pub fn reset_button(&self) -> &str {
        &self.inner.reset_button
    }
...
impl TranslationCache {
    fn new(lang: Language) -> Self {
        Self {
            ...
            double_tap_on_hover: get_raw_translation(lang, RawKey::DoubleTapOnHover).to_string(),
            double_tap_off_hover: get_raw_translation(lang, RawKey::DoubleTapOffHover).to_string(),
            ...
            reset_button: get_raw_translation(lang, RawKey::ResetBtn).to_string(),
...
fn get_raw_translation(lang: Language, key: RawKey) -> &'static str {
    match (lang, key) {
        // App Title
        (Language::English, RawKey::AppTitle) => "DfoVibration-V3 Turbo & Mapping Tool",
        (Language::SimplifiedChinese, RawKey::AppTitle) => "DfoVibration-V3 连发与映射工具",
        (Language::TraditionalChinese, RawKey::AppTitle) => "DfoVibration-V3 連發與映射工具",
        (Language::Japanese, RawKey::AppTitle) => "DfoVibration-V3 連打マッピングツール",
        ...
        (Language::English, RawKey::DoubleTapOnHover) => "Easy-run ON - one press auto-sends a second tap (for DFO running)",
        (Language::English, RawKey::DoubleTapOffHover) => "Easy-run OFF - one press sends one tap",
        (Language::SimplifiedChinese, RawKey::TurboOffHover) => "连发关闭 - 仅单次输入",
        (Language::SimplifiedChinese, RawKey::DoubleTapOnHover) => "简易奔跑开启 - 按一次自动补一次敲击 (DFO 跑动)",
        (Language::SimplifiedChinese, RawKey::DoubleTapOffHover) => "简易奔跑关闭 - 按一次触发一次按键",
        (Language::TraditionalChinese, RawKey::TurboOffHover) => "連發關閉 - 僅單次輸入",
        (Language::TraditionalChinese, RawKey::DoubleTapOnHover) => "簡易奔跑開啟 - 按一次自動補一次敲擊 (DFO 跑動)",
        (Language::TraditionalChinese, RawKey::DoubleTapOffHover) => "簡易奔跑關閉 - 按一次觸發一次按鍵",
        (Language::Japanese, RawKey::TurboOffHover) => "連打オフ - 単発入力",
        (Language::Japanese, RawKey::DoubleTapOnHover) => "簡易ダッシュON - 1回で追加タップを送信 (DFO ダッシュ用)",
        (Language::Japanese, RawKey::DoubleTapOffHover) => "簡易ダッシュOFF - 1回で1回入力",
        ...
        (Language::English, RawKey::ResetBtn) => "🔄 Reset",
        (Language::SimplifiedChinese, RawKey::ResetBtn) => "🔄 重置",
        (Language::TraditionalChinese, RawKey::ResetBtn) => "🔄 重置",
        (Language::Japanese, RawKey::ResetBtn) => "🔄 リセット",
        ...
        // Tray Icon - Activate
        (Language::English, RawKey::TrayActivate) => "✓ Activate DfoVibration-V3",
        (Language::SimplifiedChinese, RawKey::TrayActivate) => "✓ 激活 DfoVibration-V3",
        (Language::TraditionalChinese, RawKey::TrayActivate) => "✓ 啟用 DfoVibration-V3",
        (Language::Japanese, RawKey::TrayActivate) => "✓ DfoVibration-V3を有効化",

        // Tray Icon - Pause
        (Language::English, RawKey::TrayPause) => "⏸ Pause DfoVibration-V3",
        (Language::SimplifiedChinese, RawKey::TrayPause) => "⏸ 暂停 DfoVibration-V3",
        (Language::TraditionalChinese, RawKey::TrayPause) => "⏸ 暫停 DfoVibration-V3",
        (Language::Japanese, RawKey::TrayPause) => "⏸ DfoVibration-V3を一時停止",
        ...
        (Language::English, RawKey::TrayNotificationLaunched) => {
            "DfoVibration-V3 is running in the background"
        }
        (Language::SimplifiedChinese, RawKey::TrayNotificationLaunched) => "DfoVibration-V3 正在后台运行",
        (Language::TraditionalChinese, RawKey::TrayNotificationLaunched) => "DfoVibration-V3 正在背景執行",
        (Language::Japanese, RawKey::TrayNotificationLaunched) => {
            "DfoVibration-V3はバックグラウンドで実行中です"
        }

        // Tray Icon - Notification Activated
        (Language::English, RawKey::TrayNotificationActivated) => "DfoVibration-V3 activated",
        (Language::SimplifiedChinese, RawKey::TrayNotificationActivated) => "DfoVibration-V3 已激活",
        (Language::TraditionalChinese, RawKey::TrayNotificationActivated) => "DfoVibration-V3 已啟用",
        (Language::Japanese, RawKey::TrayNotificationActivated) => "DfoVibration-V3が有効になりました",

        // Tray Icon - Notification Paused
        (Language::English, RawKey::TrayNotificationPaused) => "DfoVibration-V3 paused",
        (Language::SimplifiedChinese, RawKey::TrayNotificationPaused) => "DfoVibration-V3 已暂停",
        (Language::TraditionalChinese, RawKey::TrayNotificationPaused) => "DfoVibration-V3 已暫停",
        (Language::Japanese, RawKey::TrayNotificationPaused) => "DfoVibration-V3が一時停止しました",
        ...
        (Language::SimplifiedChinese, RawKey::NoteHint) => "输入备注文字...",
        (Language::TraditionalChinese, RawKey::NoteHint) => "輸入備註文字...",
...
mod tests {
    ...
    fn test_language_all() {
        ...
        assert_eq!(all_languages[0], Language::SimplifiedChinese);
        assert_eq!(all_languages[1], Language::English);
    ...
    fn test_cached_translations_english() {
        ...
        assert!(translations.app_title().contains("DfoVibration-V3"));
