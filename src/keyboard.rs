...
/// 全局事件积压计数(背压用): dispatch 发送 +1, worker 消费 -1。
/// std mpsc 无队列长度查询, 用原子计数代替; 积压超阈值触发合并(见 turbo_worker)。
static EVENT_BACKLOG: AtomicUsize = AtomicUsize::new(0);
...
/// 原来 5ms 的短敲不被计数 (简易奔跑奔跑失效的根因, 用户实测定案)。
...
impl WorkerPool {
    ...
    fn hash_generic_device(device_type: &crate::state::DeviceType, button_id: u64) -> usize {
        ...
        // Hash device type (显式序数映射, 不读判别值内存 —— 读 discriminant 的底层
        // 表示是未定义行为, 与编译器/LTO 耦合; 显式映射保证跨版本稳定)
        let disc_value: u64 = match device_type {
            crate::state::DeviceType::Gamepad(_) => 1,
            crate::state::DeviceType::Joystick(_) => 2,
            crate::state::DeviceType::HidDevice { .. } => 3,
        };
...
impl KeyboardHook {
    pub fn new(state: Arc<AppState>) -> anyhow::Result<Self> {
        // 全局态只注册一次; supervisor 重启本线程时它已存在 —— 复用而非报错。
        // 旧实现直接 Err: hook 线程 panic 一次后重启必败, 键盘映射永久静默失效。
        let _ = crate::state::set_global_state(state.clone());
    ...
    pub fn run_message_loop(self) -> anyhow::Result<()> {
        ...
        for worker_id in 0..worker_count {
            ...
            let handle = match thread::Builder::new()
                ...
                Ok(h) => h,
                Err(e) => {
                    // 绝对稳定: 单个 worker 创建失败不 panic(否则 abort 杀进程),
                    // 记日志后跳过该 worker, 其余 worker 继续工作。
                    crate::util::crash_log("SPAWN_WORKER", &format!("turbo_worker_{worker_id}: {e}"));
                    continue;
                }
            };
        ...
        let state_clone = self.state.clone();
        let mouse_move_handle = match thread::Builder::new()
            ...
            Ok(h) => Some(h),
            Err(e) => {
                // 绝对稳定: 失败不 panic, 记日志后跳过该 worker(鼠标移动映射失效但进程稳定)
                crate::util::crash_log("SPAWN_WORKER", &format!("mouse_move_worker: {e}"));
                None
            }
        };

        if let Some(h) = mouse_move_handle {
        ...
        let exit_state = self.state.clone();
        thread::spawn(move || {
            // 绝对稳定: 等待 worker 退出但最长只等 3 秒 —— 若某个 worker 卡住
            // (长 duration 周期/异常), 不再无限等待(那会让整个程序\"退出卡死\"),
            // 超时后直接投递 WM_QUIT 结束消息循环。
            // 绝对稳定: 先等退出信号(should_exit, 由用户关窗/退出命令置位)。
            // 正常运行时 worker 本来就是“活着等待”状态, 绝不在此打扰消息循环
            // (否则启动几秒后映射会被提前终止 — 上一版缺陷已修复)。should_exit 之后:
            // 等 worker 结束但最长只等 3 秒, 超时直接投递 WM_QUIT(避免“退出卡死”)。
            while !exit_state.should_exit() {
                thread::sleep(Duration::from_millis(30));
            }
            let deadline = Instant::now() + Duration::from_millis(3000);
            let all_finished = || worker_handles.iter().all(|h| h.is_finished());
            while !all_finished() && Instant::now() < deadline {
                thread::sleep(Duration::from_millis(20));
            }
            if !all_finished() {
                crate::util::crash_log(
                    "SHUTDOWN_TIMEOUT",
                    "部分 worker 未在 3 秒内退出, 直接投递 WM_QUIT(避免退出卡死)",
                );
            }
    ...
    unsafe extern "system" fn keyboard_proc(
        ...
        if let Some(state) = crate::state::get_global_state() {
            // 绝对稳定: FFI 回调内绝不 unwind (panic 跨 FFI 边界 = abort)。
            // 与 rawinput 窗口过程同一兜底: panic 时记日志、事件放行、进程存活。
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                state.handle_key_event(w_param.0 as u32, kb_struct.vkCode)
            }));
            match result {
                Ok(true) => return LRESULT(1), // block raw key event
                Ok(false) => {}
                Err(payload) => {
                    crate::util::crash_log(
                        "FFI_KEYBOARD_HOOK_PANIC",
                        &format!(
                            "键盘 hook 回调异常, 已兜底放行(进程保持存活): {}",
                            crate::util::panic_payload_str(payload.as_ref())
                        ),
                    );
                }
            }
    ...
    fn turbo_worker(_worker_id: usize, state: Arc<AppState>, event_rx: Receiver<InputEvent>) {
        ...
        while !state.should_exit() {
            ...
            // 背压合并: 队列积压超阈值时, 逐条处理只会越追越慢(worker 可能正被
            // 长 duration 的完整注入周期占住) —— 快速清空队列、只保留每个设备
            // 最新的 按下/松开 状态后合并处理; 松开必然保留, 松手立即响应,
            // 从根上消除"无界积压 → 程序卡住/松手后狂按"。
            if unlikely(EVENT_BACKLOG.load(Ordering::Relaxed) >= 512) {
                Self::drain_and_coalesce(&state, &mut device_states, &event_rx);
                continue;
            }

            // Hot path: event receiving and processing (timeout 每次循环读取, reload 即时生效)
            let timeout_duration = Duration::from_millis(state.input_timeout());
            match event_rx.recv_timeout(timeout_duration) {
                Ok(event) => {
                    EVENT_BACKLOG.fetch_sub(1, Ordering::Relaxed);
                    Self::handle_input_event(&state, &mut device_states, event);
                }
        ...
        while let Ok(event) = event_rx.try_recv() {
            ...
            match event {
                ...
                InputEvent::Released(device) => {
                    latest.insert(device, false);
                }
            }
        }
        // 松开状态必然保留 → 松手立即停止; 仍按下的重新走正常处理路径
        for (device, pressed) in latest {
            if pressed {
                Self::handle_input_event(state, device_states, InputEvent::Pressed(device));
            } else {
                Self::handle_input_event(state, device_states, InputEvent::Released(device));
            }
        }
    }
            ...
            InputEvent::Pressed(device) => {
                    ...
                    device_states.get_mut(&device)
                {
                    if *turbo_enabled {
                        ...
                        // \"事件积压 → 松手后狂按某键 → 卡死\" 的问题。
                        // 物理松开会派发 Released 并移除缓存, 连发随即停止; 不更新
                        // last_time 以保证 handle_timeout 的节奏不被重复事件打乱。
                    } else {
                    ...
                    if let Some(mapping) = state.get_input_mapping(&device) {
                        ...
                        if double_tap_enabled {
                            ...
                            // 连招场景修复: 连发键的首次/重复按下都不再立即执行完整注入周期
                            // (press→sleep→release 会占住 worker, 高速切键时叠加积压),
                            // 统一由 handle_timeout 按 interval 节拍驱动 ——
                            // 狂按技能键来回切换只登记状态、不叠注入,
                            // 物理松开(Released 移除缓存)即停, 从根上消除"连招卡死"。
                        } else {
                            state.simulate_press(&target_action_clone);
                        }
                    }
    ...
    fn mouse_move_worker(state: Arc<AppState>, event_rx: Receiver<InputEvent>) {
        ...
        while !state.should_exit() {
            ...
            // 注入前实时校验前台进程白名单(空=全部启用):
            // 手柄/摇杆移动路径同样受白名单约束, 防止切到桌面/其它程序时
            // 鼠标被\"劫持\"自由移动(与键盘注入路径的校验保持一致)
            // 注入前实时校验前台进程白名单(空=全部启用) + 输入法打字组合暂停:
            // 手柄/摇杆移动路径同样受约束, 防止切到桌面/打字时鼠标被\"劫持\"自由移动
            if unlikely(
                !state.is_process_whitelisted()
                    || crate::state::is_ime_composing()
                    || crate::state::injection_cooldown_active()
            ) {
                active_directions = 0;
                scroll_active = 0;
                mapping_cache.clear();
    ...
    fn handle_mouse_action_event(
        ...
        match event {
            ...
            InputEvent::Released(device) => {
                ...
                // 移动与滚轮槽位对同一设备可同时登记 (多重动作映射), 两组必须都清。
                // 旧实现在此 early return, 方向位残留在 active_directions 里 →
                // 松手后鼠标持续朝该方向漂移停不下来。
