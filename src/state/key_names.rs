//! 键位/输入名称解析 + VK→扫描码映射 —— 原 state.rs (方法 3456-3959 + SCANCODE_MAP 3974-4098), 2026-09-27 架构重构 B2 归位。
//! 无 pub 修饰的方法统一放宽为 pub(super): 与拆分前 "state 模块内可见" 语义严格等价。
...
impl AppState {
    ...
    #[inline]
    pub(super) fn update_switch_key_cache(cache: &SwitchKeyCache, key_name: &str) -> anyhow::Result<()> {
        cache.clear();
        ...
        *util::write_guard(&cache.full_device) = Some(device);
        Ok(())
    }

    /// Convert button IDs to bitmask
    #[inline(always)]
    pub(super) fn inputs_to_bitset(inputs: &[u32]) -> u32 {
        inputs
            .iter()
    ...
    pub(super) fn key_name_to_vk(key_name: &str) -> Option<u32> {
    ...
    pub(super) fn vk_to_scancode(vk_code: u32) -> u16 {
    ...
    pub(super) fn parse_xinput_combo(input: &str) -> Option<InputDevice> {
    ...
    pub(super) fn parse_device_with_handle(input: &str) -> Option<InputDevice> {
    ...
    pub(super) fn input_name_to_output(name: &str) -> Option<OutputAction> {
        let name_upper = name.to_uppercase();
    ...
    pub(super) fn mouse_scroll_name_to_direction(name: &str) -> Option<MouseScrollDirection> {
        match name {
    ...
    pub(super) fn mouse_move_name_to_direction(name: &str) -> Option<MouseMoveDirection> {
        match name {
    ...
    pub(super) fn mouse_button_name_to_type(name: &str) -> Option<MouseButton> {
        match name {
