...
fn test_config_file_format_preservation() {
    ...
    // serde 序列化格式: 注释头 + 关键字段全部落盘
    assert!(content.contains("Sorahk Configuration File"));
    assert!(content.contains("show_tray_icon"));
    assert!(content.contains("rawinput_capture_mode"));
    assert!(content.contains("xinput_capture_mode"));
